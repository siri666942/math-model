...
def create_rewritten_pdf(input_path, output_path):
    """使用draw_rect覆盖+insert_text重写PDF"""
    ...
    for page_num in range(doc.page_count):
            ...
            if block.get("type") == 0:
                for line in block.get("lines", []):
                    for span in line.get("spans", []):
                        ...
                        rewritten = process_span_text(original)
        ...
        if spans_to_replace:
            print(f"  页面 {page_num+1}: 替换了 {len(spans_to_replace)} 处文本")

    doc.save(output_path, garbage=4, deflate=True)
    doc.close()
    print(f"\nPDF改写完成: 共替换 {total_replaced} 处文本")
...
def create_docx(input_pdf_path, output_docx_path):
    ...
    for page_num in range(pdf_doc.page_count):
        ...
        for block in blocks:
            ...
            rewritten = clean_xml_text(rewritten)  # 清理XML不兼容字符
    ...
    print(f"DOCX生成完成: {output_docx_path}")
...
if __name__ == "__main__":
    ...
    # 生成PDF
    print("\n[2/3] 生成改写版PDF...")
    create_rewritten_pdf(input_pdf, output_pdf)
