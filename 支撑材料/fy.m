...
obj_func([3.138978 139.9796 0      2.408497      2.567866 4.187053e-06      5.000162      5.887084])
%{
% 设置变量范围和维度
nvars = 8;
lb = [2, 122, 13, 1, 6, 1, 4, 1];
ub = [3, 123, 14, 2, 7, 2, 5, 2];
options = optimoptions('particleswarm', 'SwarmSize', 100, 'MaxStallIterations', 200, 'OutputFcn', @myOutputFcn);
...
function stop = myOutputFcn(optimValues, state)
    stop = false; % 不要停止优化
    if strcmp(state, 'iter') % 仅在迭代时输出
        disp(['迭代次数: ', num2str(optimValues.iteration)]);
        disp(['当前最优解: ', num2str(optimValues.bestx)]);
        disp(['当前最优值: ', num2str(optimValues.bestfval)]);
        disp('---');
    end
end
%}
        ...
        for i = 1:3
            if t_current < release_time(i)
                ...
                if t_current <= release_time(i) + detonation_delay(i)
                    ...
                    if bomb_h(i) == 0
                        bomb_h(i) = FY_pos(3)  - 0.5 * g * detonation_delay(i)^2;
                        smoke_pos(i, 1:2) = FY_pos0(1:2) + FY_dir(1:2) * FY_vel * (release_time(i) + detonation_delay(i));
                    end
                    % 爆炸后云团下沉
