...
lb = [3, 130,    0, 1, 1, 0, 2, 2];
ub = [3.5, 140, 2, 3, 3, 2, 6, 6];

options = optimoptions('particleswarm', 'SwarmSize', 1000, 'MaxIterations', 200, 'OutputFcn', @myOutputFcn);
        ...
        for i = 1:3
            if t_current < release_time(i)
                ...
                if t_current <= release_time(i) + detonation_delay(i)
                    ...
                    smoke_pos(i, :) = [FY1_pos(1:2), FY1_pos(3) + vertical_dist];
                else
                    % 爆炸后云团下沉
